...
from src.cruds.constraints import PARTICLE_VERSION
from src.cruds.constraints import PARTICLE_LOCATORS
...
async def create_particle(
    ...
    # 入力を DBに合わせる
    bridge = DataBridge(
        db, OBJECT_NAME, PARTICLE_LOCATORS, PARTICLE_VERSION, auth, True
    )
...
async def get_particle(
    ...
    bridge = DataBridge(db, OBJECT_NAME, PARTICLE_LOCATORS, PARTICLE_VERSION)
...
async def edit_particle(
    ...
    bridge = DataBridge(db, OBJECT_NAME, PARTICLE_LOCATORS, PARTICLE_VERSION, auth)
    patch_to_model(particle_db, model.dict(exclude_unset=True), PARTICLE_LOCATORS, [])
    await save_to_db(db, particle_db)
    bridge.to_resp(particle_db)
    item = ParticleReqResp.from_orm(particle_db)
    return GetParticleResponse(
